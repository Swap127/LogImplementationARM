...
__main  FUNCTION		         
	 ...
	 VMOV.F32 s6, #-1                 ;Loading the initial value with -1 (it can be also be done using [VNEG.F32 s6,s5])
	
	 VMOV.F32 s7, #1                  ;Loading the initial value with 1  for storing multiplication output
	 VNEG.F32 s8,s3                   ;for getting the negative value as according to my method negative value i.e (-0.5) is required
...
LOOP VCMP.F32 s1,s5                   ;Comparing between the upper limit and the loop variable respectively
	 ...
	 VMULGE.F32 s7,s7,s8              ;(-0.5)^n is calculated and stored
	 VDIVGE.F32 s9,s7,s5              ;((-0.5)^n/n)
	 VSUBGE.F32 s4,s4,s9              ;accumulation of all the terms is done and stored  
	 VADDGE.F32 s5, s5, s2 		      ; increament n(increamenting loop)
     BGE LOOP 					      ; run the loop 5 times and exit the loop when the variable becomes 6
     
	 
	 VMOV.F32 s10,s4                  ;The final value is transfered from s4 to s10 finally	 
